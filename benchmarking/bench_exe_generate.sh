...
do
    ...
    echo "Running $now"
    start_time=$(date +%s)
    ./vg-in-place --tool=cachegrind --instr-at-start=no --cache-sim=yes --D1=49152,12,64 --I1=32768,8,64 --L2=1310720,10,64 -v --cachegrind-out-file="cachegrind.out.$now" --log-fd=1  $dst/obj/$now | grep -v "-" | grep -v "=" >  $dst/trace/$now.log
    finish_time=$(date +%s)
