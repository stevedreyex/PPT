...
extern int isl_options_parse(struct isl_options *opt, int argc, char **argv, unsigned int flags);
extern isl_ctx *isl_ctx_alloc_with_options(struct isl_args *args, void *opt);

extern isl_schedule_constraints *isl_schedule_constraints_read_from_file(isl_ctx *ctx, FILE *input);
extern isl_schedule *isl_schedule_constraints_compute_schedule(isl_schedule_constraints *sc);
...
extern isl_printer *isl_printer_print_schedule(isl_printer *p, isl_schedule *schedule);
                      ...
                      IRType gWordTy, IRType hWordTy )
{
   ...
   if (file_name_scop == NULL) {
      ...
      // arg_value[0] = (char *)VG_(malloc)("/home/dreyex/Documents/Research/PPT/.in_place/cachegrind-amd64-linux", 70);
      // arg_value[1] = (char *)file_name_scop;
      // int arg_counts = 2;

      // // Set the pet pragma scop for the file
      // options = isl_options_new_with_defaults();
      // arg_counts = isl_options_parse(options, arg_counts, arg_value, ISL_ARG_ALL);
