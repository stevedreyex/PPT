...
// Not finish separarte from cachegrind / valgrind yet
// Modify this file till it can be compiled and run separately from cachegrind / valgrind
...
static VG_REGPARM(1) void log_1Ir(InstrInfo *n) { n->parent->Ir.a++; }

// Only used with --cache-sim=no.
static VG_REGPARM(2) void log_2Ir(InstrInfo *n, InstrInfo *n2) {
  n->parent->Ir.a++;
  n2->parent->Ir.a++;
}

// Only used with --cache-sim=no.
static VG_REGPARM(3) void log_3Ir(InstrInfo *n, InstrInfo *n2, InstrInfo *n3) {
  n->parent->Ir.a++;
  n2->parent->Ir.a++;
  n3->parent->Ir.a++;
}
...
static VG_REGPARM(1) void log_1IrGen_0D_cache_access(InstrInfo *n) {
  // VG_(printf)("1IrGen_0D :  CCaddr=0x%010lx,  iaddr=0x%010lx,  isize=%lu\n",
  //             n, n->instr_addr, n->instr_len);
  cachesim_I1_doref_Gen(n->instr_addr, n->instr_len, &n->parent->Ir.m1,
                        &n->parent->Ir.mL);
  n->parent->Ir.a++;
}

static VG_REGPARM(1) void log_1IrNoX_0D_cache_access(InstrInfo *n) {
...
static VG_REGPARM(2) void log_2IrNoX_0D_cache_access(InstrInfo *n,
                                                     InstrInfo *n2) {
  ...
  cachesim_I1_doref_NoX(n->instr_addr, n->instr_len, &n->parent->Ir.m1,
                        &n->parent->Ir.mL);
  n->parent->Ir.a++;
  cachesim_I1_doref_NoX(n2->instr_addr, n2->instr_len, &n2->parent->Ir.m1,
                        &n2->parent->Ir.mL);
  n2->parent->Ir.a++;
}

static VG_REGPARM(3) void log_3IrNoX_0D_cache_access(InstrInfo *n,
                                                     InstrInfo *n2,
                                                     InstrInfo *n3) {
  ...
  cachesim_I1_doref_NoX(n2->instr_addr, n2->instr_len, &n2->parent->Ir.m1,
                        &n2->parent->Ir.mL);
  n2->parent->Ir.a++;
  cachesim_I1_doref_NoX(n3->instr_addr, n3->instr_len, &n3->parent->Ir.m1,
                        &n3->parent->Ir.mL);
  n3->parent->Ir.a++;
}

static VG_REGPARM(3) void log_1IrNoX_1Dr_cache_access(InstrInfo *n,
                                                      Addr data_addr,
                                                      Word data_size) {
...
static VG_REGPARM(3) void log_1IrNoX_1Dw_cache_access(InstrInfo *n,
                                                      Addr data_addr,
                                                      Word data_size) {
...
static VG_REGPARM(3) void log_0Ir_1Dr_cache_access(InstrInfo *n, Addr data_addr,
                                                   Word data_size) {
...
static VG_REGPARM(3) void log_0Ir_1Dw_cache_access(InstrInfo *n, Addr data_addr,
                                                   Word data_size) {
...
static VG_REGPARM(2) void log_cond_branch(InstrInfo *n, Word taken) {
  // VG_(printf)("cbrnch:  CCaddr=0x%010lx,  taken=0x%010lx\n",
  //             n, taken);
  n->parent->Bc.b++;
  n->parent->Bc.mp += (1 & do_cond_branch_predict(n->instr_addr, taken));
}

static VG_REGPARM(2) void log_ind_branch(InstrInfo *n, UWord actual_dst) {
  // VG_(printf)("ibrnch:  CCaddr=0x%010lx,    dst=0x%010lx\n",
  //             n, actual_dst);
  n->parent->Bi.b++;
  n->parent->Bi.mp += (1 & do_ind_branch_predict(n->instr_addr, actual_dst));
}
...
static SB_info *get_SB_info(IRSB *sbIn, Addr origAddr) {
  Int i, n_instrs;
  IRStmt *st;
  SB_info *sbInfo;
  ...
  sbInfo = VG_(OSetGen_AllocNode)(
      instrInfoTable, sizeof(SB_info) + n_instrs * sizeof(InstrInfo));
...
static InstrInfo *setup_InstrInfo(CgState *cgs, Addr instr_addr,
                                  UInt instr_len) {
  InstrInfo *i_node;
...
static void flushEvents(CgState *cgs) {
  ...
  while (i < cgs->events_used) {
    ...
    case Ev_IrNoX:
      ...
      else if (ev2 && ev3 && ev2->tag == Ev_IrNoX && ev3->tag == Ev_IrNoX) {
        if (clo_cache_sim) {
      ...
      /* Merge an IrNoX with one following IrNoX. */
        ...
        argv = mkIRExprVec_2(i_node_expr, mkIRExpr_HWord((HWord)ev2->inode));
        regparms = 2;
        i += 2;
      }
...
static void addEvent_Dr(CgState *cgs, InstrInfo *inode, Int datasize,
                        IRAtom *ea) {
...
static void addEvent_Dw(CgState *cgs, InstrInfo *inode, Int datasize,
                        IRAtom *ea) {
  tl_assert(isIRAtom(ea));
...
static void addEvent_D_guarded(CgState *cgs, InstrInfo *inode, Int datasize,
                               IRAtom *ea, IRAtom *guard, Bool isWrite) {
  tl_assert(isIRAtom(ea));
  tl_assert(guard);
  ...
  helperName =
      isWrite ? "log_0Ir_1Dw_cache_access" : "log_0Ir_1Dr_cache_access";
  helperAddr = isWrite ? &log_0Ir_1Dw_cache_access : &log_0Ir_1Dr_cache_access;
...
static void addEvent_Bc(CgState *cgs, InstrInfo *inode, IRAtom *guard) {
  Event *evt;
  tl_assert(isIRAtom(guard));
...
static void addEvent_Bi(CgState *cgs, InstrInfo *inode, IRAtom *whereTo) {
  Event *evt;
...
static IRSB *cg_instrument(VgCallbackClosure *closure, IRSB *sbIn,
  ...
  for (/*use current i*/; i < sbIn->stmts_used; i++) {
    ...
    case Ist_WrTmp: {
      ...
      if (data->tag == Iex_Load) {
        ...
        addEvent_Dr(&cgs, curr_inode, sizeofIRType(data->Iex.Load.ty), aexpr);
      }
      break;
    }
    ...
    case Ist_Store: {
      ...
      addEvent_Dw(&cgs, curr_inode, sizeofIRType(typeOfIRExpr(tyenv, data)),
                  aexpr);
      break;
    }
    ...
    case Ist_StoreG: {
      ...
      addEvent_D_guarded(&cgs, curr_inode, sizeofIRType(type), addr, sg->guard,
                         True /*isWrite*/);
      break;
    }
    ...
    case Ist_LoadG: {
      ...
      tl_assert(type != Ity_INVALID);
      addEvent_D_guarded(&cgs, curr_inode, sizeofIRType(type), addr, lg->guard,
                         False /*!isWrite*/);
      break;
    }
    ...
    case Ist_LLSC: {
      ...
      if (st->Ist.LLSC.storedata == NULL) {
        ...
        addEvent_Dr(&cgs, curr_inode, sizeofIRType(dataTy), st->Ist.LLSC.addr);
        /* flush events before LL, should help SC to succeed */
        flushEvents(&cgs);
      } else {
        /* SC */
        dataTy = typeOfIRExpr(tyenv, st->Ist.LLSC.storedata);
        addEvent_Dw(&cgs, curr_inode, sizeofIRType(dataTy), st->Ist.LLSC.addr);
      }
      break;
    }
    ...
    case Ist_Exit: {
      ...
      if ((st->Ist.Exit.jk == Ijk_Boring) || (st->Ist.Exit.jk == Ijk_Call) ||
        ...
        addStmtToIRSB(cgs.sbOut, IRStmt_WrTmp(guard1, st->Ist.Exit.guard));
        addStmtToIRSB(
            cgs.sbOut,
        ...
        addStmtToIRSB(
            cgs.sbOut,
            IRStmt_WrTmp(
                guard, inverted ? IRExpr_Binop(opXOR, IRExpr_RdTmp(guardW), one)
                                : IRExpr_RdTmp(guardW)));
        /* And post the event. */
...
static void fprint_CC_table_and_calc_totals(void) {
  ...
  while ((lineCC = VG_(OSetGen_Next)(CC_table))) {
    ...
    if (clo_cache_sim && clo_branch_sim) {
      ...
      (fp,
       ...
       " %llu %llu %llu"
       " %llu %llu %llu %llu\n",
       lineCC->loc.line, lineCC->Ir.a, lineCC->Ir.m1, lineCC->Ir.mL,
       lineCC->Dr.a, lineCC->Dr.m1, lineCC->Dr.mL, lineCC->Dw.a, lineCC->Dw.m1,
       lineCC->Dw.mL, lineCC->Bc.b, lineCC->Bc.mp, lineCC->Bi.b, lineCC->Bi.mp);
    } else if (clo_cache_sim && !clo_branch_sim) {
      VG_(fprintf)
      (fp,
       "%d %llu %llu %llu"
      ...
      (fp,
       "%d %llu"
       " %llu %llu %llu %llu\n",
       lineCC->loc.line, lineCC->Ir.a, lineCC->Bc.b, lineCC->Bc.mp,
       lineCC->Bi.b, lineCC->Bi.mp);
    } else {
      VG_(fprintf)(fp, "%d %llu\n", lineCC->loc.line, lineCC->Ir.a);
    }
  ...
  if (clo_cache_sim && clo_branch_sim) {
    ...
    (fp,
     ...
     " %llu %llu %llu"
     " %llu %llu %llu %llu\n",
     Ir_total.a, Ir_total.m1, Ir_total.mL, Dr_total.a, Dr_total.m1, Dr_total.mL,
     Dw_total.a, Dw_total.m1, Dw_total.mL, Bc_total.b, Bc_total.mp, Bi_total.b,
     Bi_total.mp);
  } else if (clo_cache_sim && !clo_branch_sim) {
    VG_(fprintf)
    (fp,
    ...
    (fp,
     ...
     " %llu"
     " %llu %llu %llu %llu\n",
     Ir_total.a, Bc_total.b, Bc_total.mp, Bi_total.b, Bi_total.mp);
  } else {
    VG_(fprintf)
    (fp,
     "summary:"
...
#define CG_MAX(a, b) ((a) >= (b) ? (a) : (b))
  ...
  if (clo_cache_sim) {
    ...
    if (0 == Ir_total.a)
      Ir_total.a = 1;
    VG_(umsg)("I1  miss rate: %*.2f%%\n", l1, Ir_total.m1 * 100.0 / Ir_total.a);
    VG_(umsg)("LLi miss rate: %*.2f%%\n", l1, Ir_total.mL * 100.0 / Ir_total.a);
    VG_(umsg)("\n");
    ...
    /* Make format string, getting width right for numbers */
    VG_(sprintf)
    (fmt, "%%s %%,%dllu  (%%,%dllu rd   + %%,%dllu wr)\n", l1, l2, l3);

    VG_(umsg)(fmt, "D refs:       ", D_total.a, Dr_total.a, Dw_total.a);
    ...
    ("D1  miss rate: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1,
     D_total.m1 * 100.0 / D_total.a, l2, Dr_total.m1 * 100.0 / Dr_total.a, l3,
     Dw_total.m1 * 100.0 / Dw_total.a);
    VG_(umsg)
    ("LLd miss rate: %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1,
     D_total.mL * 100.0 / D_total.a, l2, Dr_total.mL * 100.0 / Dr_total.a, l3,
     Dw_total.mL * 100.0 / Dw_total.a);
    VG_(umsg)("\n");
    ...
    VG_(umsg)(fmt, "LL refs:      ", LL_total, LL_total_r, LL_total_w);

    LL_total_m = Dr_total.mL + Dw_total.mL + Ir_total.mL;
    LL_total_mr = Dr_total.mL + Ir_total.mL;
    LL_total_mw = Dw_total.mL;
    VG_(umsg)(fmt, "LL misses:    ", LL_total_m, LL_total_mr, LL_total_mw);

    VG_(umsg)
    ("LL miss rate:  %*.1f%% (%*.1f%%     + %*.1f%%  )\n", l1,
     LL_total_m * 100.0 / (Ir_total.a + D_total.a), l2,
     LL_total_mr * 100.0 / (Ir_total.a + Dr_total.a), l3,
     LL_total_mw * 100.0 / Dw_total.a);
  }
  ...
  if (clo_branch_sim) {
    ...
    VG_(umsg)("\n");
    VG_(umsg)(fmt, "Branches:     ", B_total.b, Bc_total.b, Bi_total.b);

    VG_(umsg)(fmt, "Mispredicts:  ", B_total.mp, Bc_total.mp, Bi_total.mp);

    VG_(umsg)
    ("Mispred rate:  %*.1f%% (%*.1f%%     + %*.1f%%   )\n", l1,
     B_total.mp * 100.0 / B_total.b, l2, Bc_total.mp * 100.0 / Bc_total.b, l3,
     Bi_total.mp * 100.0 / Bi_total.b);
  }

  // Various stats
  if (VG_(clo_stats)) {
    Int debug_lookups = full_debugs + fn_debugs + file_line_debugs + no_debugs;
...
static void cg_discard_superblock_info(Addr orig_addr64, VexGuestExtents vge) {
...
static Bool cg_process_cmd_line_option(const HChar *arg) {
  if (VG_(str_clo_cache_opt)(arg, &clo_I1_cache, &clo_D1_cache,
                             &clo_LL_cache)) {
  }
...
static void cg_print_usage(void) {
  VG_(printf)
  ("    --cachegrind-out-file=<file>     output file name "
   "[cachegrind.out.%%p]\n"
   "    --cache-sim=yes|no               collect cache stats? [no]\n"
   "    --branch-sim=yes|no              collect branch prediction stats? "
   "[no]\n"
...
static void set_instr_enabled(Bool enable) {
  if (enable) {
    ...
    if (!instr_enabled) {
      ...
      VG_(discard_translations_safely)
      ((Addr)0x1000, ~(SizeT)0xfff, "cachegrind");
      instr_enabled = True;
    } else {
...
static Bool cg_handle_client_request(ThreadId tid, UWord *args, UWord *ret) {
  ...
  default:
    VG_(message)
    (Vg_UserMsg, "Warning: unknown cachegrind client request code %llx\n",
     (ULong)args[0]);
    return False;
  }
}
...
static void cg_pre_clo_init(void) {
  // VG_(message)(Vg_ClientMsg, "- Cachegrind tool, part of Valgrind\n");
  ...
  VG_(basic_tool_funcs)(cg_post_clo_init, cg_instrument, cg_fini);

  VG_(needs_superblock_discards)(cg_discard_superblock_info);
  VG_(needs_command_line_options)
  (cg_process_cmd_line_option, cg_print_usage, cg_print_debug_usage);
...
static void cg_post_clo_init(void) {
  ...
  CC_table = VG_(OSetGen_Create)(offsetof(LineCC, loc), cmp_CodeLoc_LineCC,
                                 VG_(malloc), "cg.main.cpci.1", VG_(free));
  instrInfoTable = VG_(OSetGen_Create)(/*keyOff*/ 0, NULL, VG_(malloc),
                                       "cg.main.cpci.2", VG_(free));
  stringTable = VG_(OSetGen_Create)(/*keyOff*/ 0, stringCmp, VG_(malloc),
                                    "cg.main.cpci.3", VG_(free));

  if (clo_cache_sim) {
    VG_(post_clo_init_configure_caches)
    (&I1c, &D1c, &LLc, &clo_I1_cache, &clo_D1_cache, &clo_LL_cache);
